...
ADMISSION_ALPHA_MIN = 0.005
ADMISSION_ALPHA_MAX = 0.2
ADMISSION_ALPHA_FLOOR_MULT = 0.7
ADMISSION_USE_CAPACITY_SCALE = True
...
def run_single_capacity(
    ...
    if base_learner != "nb":
        raise ValueError("run_il_cache_overhead_no_guard.py supports only base_learner='nb'.")
    ...
    def _finalize_slot() -> None:
        ...
        # IL-no-guard disables Guardrails precision feedback and score-quality
        # modulation, but keeps the delayed Top-M skeleton for fair comparison.
        budget_start = now_ns()
        candidate_score_list = [candidate_scores[obj_id] for obj_id in candidate_ids]
        if quality_active and miss_candidates > 0:
...
def run_experiment(
    ...
    if model_name is None:
        model_name = f"ilnse_{feature_set}_guard_no_guard_{base_learner}_overhead"
        if impl_mode == "optimized":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="IL no-guard overhead runner (slot-level timing).")
    ...
    try:
        ...
        error_path = os.path.join(
            args.results_root,
            f"error_il_no_guard_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json",
        )
