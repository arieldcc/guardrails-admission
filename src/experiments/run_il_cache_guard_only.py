...
def run_single_capacity(
    ...
    def _finalize_slot() -> None:
        ...
        if miss_candidates > 0:
            ...
            global_quality = pq - p50
            score_spread = global_quality
        if miss_candidates > 0 and GUARD_QUALITY_INTERACTION_MODE != "boundary_cap_modulation":
            ...
            if GUARD_QUALITY_INTERACTION_MODE == "conditional":
                guard_precision_for_quality = precision_eff if precision_eff is not None else admit_precision
                use_quality = (
                    guard_precision_for_quality is None
                    or guard_precision_for_quality < ADMISSION_PRECISION_TARGET
                    or quality_mult < 1.0
                )
                if not use_quality:
                    quality_mult = 1.0
        ...
        score_acc = float("nan")
        precision_budget_before_quality_cap = 0
        ...
        precision_pressure_mult = 1.0 + PRESSURE_MISS_GAMMA * miss_rate
        precision_budget_before_quality_cap = _compute_admission_budget(
            slot_cache_misses,
            capacity_objects,
            alpha_for_slot,
            precision_pressure_mult,
        )
        if fill_phase and precision_budget_before_quality_cap < fill_min_budget:
            precision_budget_before_quality_cap = fill_min_budget
        if miss_candidates <= 0:
            precision_budget_before_quality_cap = 0
        elif precision_budget_before_quality_cap > miss_candidates:
            precision_budget_before_quality_cap = miss_candidates

        if (
            GUARD_QUALITY_INTERACTION_MODE == "boundary_cap_modulation"
            and miss_candidates > 1
            and candidate_score_list
        ):
            sorted_scores = sorted((float(score) for score in candidate_score_list), reverse=True)
            k = min(max(1, precision_budget_before_quality_cap), miss_candidates)
            r = max(1, int(math.ceil(SCORE_GATE_TOP_PERCENT * miss_candidates)))
            low_rank = min(miss_candidates, k + r)
            boundary_margin = sorted_scores[k - 1] - sorted_scores[low_rank - 1]
            q75 = float(np.quantile(sorted_scores, 0.75))
            q25 = float(np.quantile(sorted_scores, 0.25))
            iqr = q75 - q25
            boundary_quality = boundary_margin / (iqr + SCORE_SPREAD_EPS)
            if score_spread_ema is None:
                score_spread_ema = boundary_quality
            else:
                score_spread_ema = (
                    SCORE_SPREAD_EMA_ALPHA * boundary_quality
                    + (1.0 - SCORE_SPREAD_EMA_ALPHA) * score_spread_ema
                )
            denom = (score_spread_ema if score_spread_ema is not None else 0.0) + SCORE_SPREAD_EPS
            quality_mult = boundary_quality / denom
            if quality_mult < SCORE_QUALITY_MIN:
                quality_mult = SCORE_QUALITY_MIN
            elif quality_mult > SCORE_QUALITY_MAX:
                quality_mult = SCORE_QUALITY_MAX
            score_spread = boundary_quality
            score_spread_sum += score_spread
            score_spread_count += 1
            quality_mult_sum += quality_mult
            quality_mult_count += 1
        ...
        if is_cache_slot:
            ...
            quality_budget_mult = (
                1.0
                if GUARD_QUALITY_INTERACTION_MODE in ("cap_modulation", "boundary_cap_modulation")
                else quality_mult
            )
            pressure_mult = (1.0 + PRESSURE_MISS_GAMMA * miss_rate) * quality_budget_mult
        ...
        if miss_candidates > 0:
            score_gate_percent = SCORE_GATE_TOP_PERCENT
            if GUARD_QUALITY_INTERACTION_MODE in ("cap_modulation", "boundary_cap_modulation"):
                score_gate_percent = SCORE_GATE_TOP_PERCENT * quality_mult
                if score_gate_percent < QUALITY_CAP_MIN:
                    score_gate_percent = QUALITY_CAP_MIN
                elif score_gate_percent > QUALITY_CAP_MAX:
                    score_gate_percent = QUALITY_CAP_MAX
            eta_top_t = score_gate_percent
            score_gate_k = max(1, int(math.ceil(score_gate_percent * miss_candidates)))
